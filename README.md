<h1 align="center">Ajay Mungase</h1>
<h3 align="center">Full Stack Java Developer | Spring Framework | Spring Boot | REST APIs | SQL</h3>

<p align="center">
Building Scalable Backend Systems & Real-World Web Applications
</p>

---

## 🚀 Professional Snapshot

• Full Stack Java Developer focused on Spring Boot & REST APIs  
• Strong in Backend Development & Database Design  
• Experience building end-to-end web applications  
• Passionate about clean code & scalable architecture  

📍 Pune, India  

---

## 🛠 Tech Stack

**Backend:** Java, Spring Framework, Spring Boot, Hibernate, JPA, JDBC, REST APIs  
**Frontend:** HTML5, CSS3, JavaScript, React.js  
**Database:** MySQL, PostgreSQL  
**Tools:** Git, GitHub, Postman, IntelliJ, Eclipse, VS Code  

---

# 📂 Key Projects

### 🔹 RecruitEase – Recruitment Management System  
Spring Boot based recruitment platform with job posting, candidate tracking & admin dashboard.  
**Tech:** Spring Boot, Hibernate, MySQL, REST API, React.Js 

---

### 🔹 ExpenseTracker – Finance Management App  
Track daily expenses with category-based reports & secure database integration.  
**Tech:**Core Java, OOP, Exception Handling, JDBC, MySQL  

---

### 🔹 CareTrack – Healthcare Management System  
Patient records & appointment management system with backend validation.  
**Tech:** Spring Boot, Hibernate, MySQL  

---

### 🔹 SmartLearn – E-Learning Platform  
E-Learning System for Students with specific free Certified Courses.  
**Tech:** HTML, CSS, Javascript  
🌐 Live: https://ajaymungase.github.io/SmartLearn/

---

### 🔹 Personal Portfolio  
Responsive portfolio showcasing projects & skills.  
🌐 Live: https://ajaymungase.github.io/Portfolio/  

---

## 🌐 Professional Links

🔹 **Portfolio Website:**   [Click to Open](https://ajaymungase.github.io/Portfolio/)
   

🔹 **Resume (PDF):**  [Download My CV](./Ajay_Mungase_Java.pdf)
    

---

## 📫 Contact

📧 Email: mungaseajay03@gmail.com  
🔗 GitHub: https://github.com/ajaymungase  
🔗 LinkedIn: https://www.linkedin.com/in/ajaymungase  

---

⭐ Open to Full Stack Java Developer Opportunities
